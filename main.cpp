...
public:
    explicit crawl_report(net::io_context &ioc)
            : ioc_(ioc), strand_(ioc_.get_executor()), index_(0), hosts_(urls_large_data()) {
    }
...
public:
    ...
    worker(crawl_report &report, net::io_context &ioc) : report_(report), resolver_(net::make_strand(ioc)),
